...
echo curl -o -X PATCH "https://api.netlify.com/api/v1/accounts/$TEST_ID/env/DATABASE_URL?site_id=$SITE_ID" -d '{"context": "branch", "context_parameter": "test", "value": "test"}' -H "Content-Type: application/json" -H "Authorization: Bearer ${TEST_ACCESS_TOKEN}"
...
# curl -X PATCH "$URL" -d "${BODY}" -H "Content-Type: application/json" -H "Authorization: Bearer $TEST_ACCESS_TOKEN"
